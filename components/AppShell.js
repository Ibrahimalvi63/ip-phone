  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-[#0f0f0f] overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab === 'dialer' && (
          <DialerTab twilio={twilio} prefillNumber={dialTarget} onPrefillConsumed={() => setDialTarget('')} />
        )}
        {activeTab === 'calls' && (
          <CallsTab onDial={handleDialFromHistory} />
        )}
        {activeTab === 'contacts' && (
          <ContactsTab onDial={handleDialFromHistory} />
        )}
        {activeTab === 'settings' && (
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1 py-3 text-[10px] font-semibold uppercase tracking-wider transition-colors duration-150 ${
                activeTab === tab.id ? 'text-[#00ff88]' : 'text-gray-600 hover:text-gray-400'
              }`}
            >
